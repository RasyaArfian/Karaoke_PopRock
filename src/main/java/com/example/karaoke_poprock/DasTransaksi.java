...
//            lblDurasi.setText(String.valueOf(tglTransaksiRuangan));
    ...
    @FXML
    private TextField tfTanggalPembuatan, tfNamaMember, tfNoTelpMember, tfDiskonTransaksiMember, tfTotalPembayaran;
    @FXML ComboBox<String> cbTipeMember;
    @FXML
    private Label lblTarif, lblKembalianMember;
    ...
    private void loadDataTipeMember() {
        try {
            ...
            ResultSet rs = db.stat.executeQuery("SELECT id_member, tipe_member, diskon_member, harga_member FROM master_member");

            cbTipeMember.getItems().clear();
            tipeMemberMap.clear();

            while (rs.next()) {
                int id = rs.getInt("id_member");
                String nama = rs.getString("tipe_member");
                double diskon = rs.getDouble("diskon_member");
                double harga = rs.getDouble("harga_member");
